...
/**
 * MobileBottomNav - Instagram-style bottom navigation for mobile devices
 * 
 * Features:
 * - Fixed position at bottom of screen on mobile only (hidden on desktop)
 * - Main navigation items in horizontal layout
 * - Secondary actions (settings, logout) in separate row
...
const MobileBottomNav: React.FC<MobileBottomNavProps> = ({
  ...
  // Filter out settings and profile items for main navigation
  const mainNavItems = sidebarItems.filter(item => 
    item.id !== "settings" && item.id !== "profile"
  );

  // Show only first 4 items in main navigation, rest go to overflow menu
  ...
  return (
    ...
    <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 z-50 md:hidden safe-area-pb">
      ...
      <div className={`flex items-center py-2 px-2 ${overflowNavItems.length > 0 ? 'justify-around' : 'justify-evenly'}`}>
        ...
        {/* Overflow Menu Button - Only show if there are overflow items */}
        ...
        {/* {showUserProfile && (
          <div className="flex flex-col items-center justify-center p-2 min-w-0 flex-1">
            ...
            >
              <UserCircleIcon
                className={`h-6 w-6 mb-1 ${
                  activeTab === "profile" ? "text-cargo-600" : "text-gray-400"
                }`}
              />
      ...
      {/* Secondary Actions Row (Settings & Logout) */}
      {showUserProfile && (
        <div className="flex items-center justify-center space-x-4 py-2 px-4 border-t border-gray-100">
          {showSettings && (
            <button
              onClick={() => handleItemClick("settings")}
              className={`flex items-center justify-center p-2 rounded-lg transition-all duration-200 touch-manipulation ${
                activeTab === "settings"
                ...
                {overflowNavItems.map((item) => {
                  ...
                  return (
                    <button
                      key={item.id}
                      onClick={() => handleItemClick(item.id)}
                      className={`w-full flex items-center space-x-3 p-3 rounded-lg transition-all duration-200 touch-manipulation ${
                        activeTab === item.id
