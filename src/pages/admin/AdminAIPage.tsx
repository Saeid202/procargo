...
const AdminAIPage: React.FC = () => {
  ...
  const [categoryInstructions, setCategoryInstructions] = useState<
    Record<string, string>
  >({
    Electronics:
      "Focus on electronic components, safety certifications (CSA, FCC), and potential ITAR restrictions.",
    "Textiles & Apparel":
      "Consider textile quotas, labeling requirements, and country of origin marking.",
    "Machinery & Equipment":
      "Check for safety standards, electrical certifications, and potential dual-use restrictions.",
    Chemicals:
      "Emphasize chemical safety data sheets, environmental regulations, and controlled substances.",
    "Food & Beverages":
      "Focus on food safety regulations, labeling requirements, and import permits.",
    Automotive:
      "Consider automotive safety standards, emissions requirements, and recall information.",
    Pharmaceuticals:
      "Emphasize Health Canada approvals, controlled substances, and prescription requirements.",
    "Construction Materials":
      "Check building codes, safety standards, and environmental impact.",
    "Agricultural Products":
      "Consider plant health certificates, organic certifications, and seasonal restrictions.",
    Other:
      "Apply general customs principles and recommend specific research areas.",
  });
  ...
  const handleSetActive = async (id: string) => {
    const { error } = await AIConfigService.setActiveConfig(id);
    if (error) {
      toast.error(error);
    } else {
  ...
  const handleDelete = async (id: string) => {
    if (
      window.confirm("Are you sure you want to delete this AI configuration?")
    ) {
      const { error } = await AIConfigService.deleteConfig(id);
      if (error) {
        toast.error(error);
      } else {
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-600">
            Manage AI behavior and analysis settings for compliance analysis
          </p>
        </div>
        <button
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    {Object.entries(categoryInstructions).map(
                      ([category, instruction]) => (
                        <div key={category} className="flex space-x-2">
                          <label className="w-32 text-sm text-gray-600 flex-shrink-0 mt-2">
                            {category}:
                          </label>
                          <textarea
                            value={instruction}
                            onChange={(e) =>
                              setCategoryInstructions((prev) => ({
                                ...prev,
