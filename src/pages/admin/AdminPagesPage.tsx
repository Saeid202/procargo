...
import { createDefaultPages } from "../../utils/testPages";
import {
  PlusIcon,
  PencilIcon,
  TrashIcon,
...
const AdminPagesPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingPage) {
        const { error } = await ContentManagementService.updatePage(
          editingPage.id,
          formData
        );
        if (error) {
        ...
        const { error } = await ContentManagementService.createPage({
          ...formData,
  ...
  const handleDelete = async (id: string) => {
    if (window.confirm("Are you sure you want to delete this page?")) {
      const { error } = await ContentManagementService.deletePage(id);
      if (error) {
        toast.error(error);
      } else {
        ...
        {pages.length === 0 ? (
          <div className="text-center py-12">
            ...
            <p className="mt-1 text-sm text-gray-500">
              Get started by creating your first page or add the default pages.
            </p>
                  ...
                  <tr key={page.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                      <div className="flex space-x-2">
                        <button
                          onClick={() =>
                            window.open(`/${page.page_slug}`, "_blank")
                          }
