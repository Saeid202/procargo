...
import { ContentManagementService, SiteSettings } from '../../services/contentManagementService';
...
const AdminSiteSettingsPage: React.FC = () => {
  ...
  const loadSiteSettings = async () => {
    setLoading(true);
    const { settings: data, error } = await ContentManagementService.getSiteSettings();
    if (error) {
      toast.error(error);
    } else {
      setSettings(data);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const { success, error } = await ContentManagementService.updateSiteSettings(formData);
      if (error) {
        toast.error(error);
      } else {
    ...
    <div className="space-y-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="bg-white border border-gray-200 rounded-xl shadow-sm p-6">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">Analytics Code</label>
            <textarea
            ...
            <p className="mt-2 text-sm text-gray-500">
              Paste your Google Analytics, Google Tag Manager, or other tracking code here.
            </p>
          </div>
        </div>
