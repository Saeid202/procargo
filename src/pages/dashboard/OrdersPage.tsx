...
const OrdersPage: React.FC = () => {
    ...
    setFormData((prev) => ({
      ...
      suppliers: prev.suppliers.map((s) =>
        s.id === supplierId
          ? {
              ...s,
              supplier_links: s.supplier_links.filter(
                (_, i) => i !== linkIndex
              ),
            }
          : s
  ...
  const handleOtherRequestSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const requestData = {
        ...
        urgency: otherRequestData.urgency as
          | "low"
          | "normal"
          | "high"
          | "urgent",
      ...
      const { request, error: requestError } =
        await OtherRequestService.createOtherRequest(user.id, requestData);
  ...
  const handleQuotationRequestSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { quotation, error: quotationError } =
        await QuotationService.createQuotationRequest(user.id, quotationData);
      ...
      if (quotationRequestData.productImages.length > 0) {
        for (const image of quotationRequestData.productImages) {
          const { error: uploadError } =
            await QuotationService.uploadQuotationFile(
              quotation.id,
              user.id,
              image
            );
      ...
      // Show success message
      alert(
        "Your quotation request has been submitted successfully! Our agents will contact you soon with quotes."
      );
    } catch (error) {
  ...
  const handleOrderSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 1. Create order
      const { order, error: orderError } = await SupabaseService.createOrder(
        user.id,
        formData,
        orderNumber
      );
      ...
      for (const supplier of formData.suppliers) {
        ...
        if (supplier.supplier_links?.length) {
          const { error: linksError } =
            await SupabaseService.createSupplierLinks(
              savedSupplier.id,
              supplier.supplier_links
            );
        ...
        if (supplier.files?.length > 0) {
          const { error: fileError } =
            await SupabaseService.uploadSupplierFiles(
              order.id,
              savedSupplier.id,
              supplier.files
            );
  ...
  const validateForm = (): boolean => {
    ...
    formData.suppliers.forEach((supplier, i) => {
      if (!supplier.product_name.trim())
        newErrors[`supplier-${i}-product_name`] = "Product name is required";
      if (supplier.quantity <= 0)
        newErrors[`supplier-${i}-quantity`] = "Quantity must be greater than 0";

      if (!supplier.supplier_links.length) {
        newErrors[`supplier-${i}-link`] =
          "At least one supplier link is required";
      } else {
        supplier.supplier_links.forEach((link, j) => {
          if (!link.url.trim()) {
            newErrors[`supplier-${i}-link-${j}-url`] =
              "Supplier link URL is required";
          }
          if (link.quantity <= 0) {
            newErrors[`supplier-${i}-link-${j}-quantity`] =
