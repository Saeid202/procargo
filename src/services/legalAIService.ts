  ...
  private static buildDeepSeekHeaders(): Record<string, string> {
    ...
    if (
      ...
      if (!this.deepSeekKeyWarningLogged) {
        console.warn(
          "DeepSeek API key is not configured. Set REACT_APP_DEEPSEEK_API_KEY to enable authorized requests."
        );
...
Respond entirely in the same language as the user's latest message. Do not switch languages or translate unless the user explicitly asks.`;
  ...
  static async sendMessage(
    ...
    try {
      ...
      const { config: aiConfig, error: configError } =
        await AIConfigService.getActiveConfig();

      if (configError || !aiConfig) {
        console.warn(
          "No AI configuration found, using direct API call:",
          configError
        );
   ...
   */
  private static async getAILegalResponse(
    message: string,
    config: any
...
${this.buildLanguageDirective(message)}`,
   ...
   */
  private static getDefaultLegalResponse(
    message: string,
    sessionId?: string
  ): {
    response: LegalAIResponse;
    ...
    if (messageLower.includes("export") || messageLower.includes("china")) {
      if (isPersian) {
        response =
          "برای صادرات از چین، لازم است فاکتور تجاری، لیست بسته‌بندی، گواهی مبدأ (در صورت نیاز) و مجوز صادرات برای کالاهای محدود داشته باشید. بررسی و تأیید اسناد معمولاً ۳ تا ۵ روز کاری زمان می‌برد.";
        suggestions = [
          "چک‌لیست مدارک صادرات",
          "مقررات صادراتی چین",
          "الزامات گواهی مبدأ",
        ];
        ...
        response =
          "For China exports, you'll need a Commercial Invoice, Packing List, Certificate of Origin (if applicable), and Export License for restricted items. The process typically takes 3-5 business days for documentation review.";
        suggestions = [
      ...
      if (isPersian) {
        response =
          "برای واردات به کانادا باید بارنامه، فاکتور تجاری به زبان انگلیسی یا فرانسوی، لیست بسته‌بندی و اظهارنامه واردات (فرم B3) ارائه کنید. ترخیص گمرک معمولاً ۱ تا ۳ روز کاری زمان می‌برد.";
        suggestions = [
          "مدارک واردات",
          "رویه‌های گمرکی کانادا",
          "الزامات فرم B3",
        ];
        relatedTopics = ["رویه‌های واردات", "ترخیص گمرک", "تجارت با کانادا"];
      } else {
        response =
          "Canada imports require a Bill of Lading, Commercial Invoice in English/French, Packing List, and Import Declaration (B3 Form). Customs clearance usually takes 1-3 business days.";
        suggestions = [
          "Import Documentation",
          "Canada Customs Procedures",
          "B3 Form Requirements",
        ];
        relatedTopics = [
          "Import Procedures",
          "Customs Clearance",
      ...
      messageLower.includes("customs") ||
      messageLower.includes("documentation")
    ) {
      if (isPersian) {
        response =
          "مدارک ضروری گمرکی شامل فاکتور تجاری، لیست بسته‌بندی، بارنامه، گواهی مبدأ و اظهارنامه گمرکی است. برای جلوگیری از تأخیر، حتماً مدارک را کامل و دقیق تکمیل کنید.";
        suggestions = ["چک‌لیست مدارک", "الزامات گمرکی", "خطاهای رایج"];
        relatedTopics = ["مدارک", "گمرک", "انطباق مقرراتی"];
      } else {
        response =
          "Essential customs documents include: Commercial Invoice, Packing List, Bill of Lading, Certificate of Origin, and Customs Declaration. Always ensure documents are complete and accurate to avoid delays.";
        suggestions = [
      ...
      messageLower.includes("tariff") ||
      messageLower.includes("duty")
    ) {
      if (isPersian) {
        response =
          "تعرفه‌ها و عوارض بر اساس طبقه‌بندی کالا (کد HS)، کشور مبدأ و توافقنامه‌های تجاری متفاوت است. برای محاسبه دقیق، از ماشین‌حساب تعرفه آژانس خدمات مرزی کانادا (CBSA) استفاده کنید. در صورت وجود توافقنامه تجاری، می‌توانید از نرخ‌های ترجیحی بهره‌مند شوید.";
        suggestions = [
          "جستجوی کد HS",
          "ماشین‌حساب تعرفه",
          "مزایای توافقنامه‌های تجاری",
        ];
        relatedTopics = [
          "تعرفه‌ها",
          "عوارض",
        ...
        response =
          "Tariffs and duties vary by product classification (HS code), country of origin, and trade agreements. Use the Canada Border Services Agency (CBSA) tariff calculator for accurate rates. Preferential rates may apply under trade agreements.";
        suggestions = [
          "HS Code Lookup",
          "Tariff Calculator",
      ...
      messageLower.includes("hs code") ||
      messageLower.includes("classification")
    ) {
      if (isPersian) {
        response =
          "کدهای HS شامل ۶ تا ۱۰ رقم هستند و برای طبقه‌بندی کالاها در گمرک استفاده می‌شوند. طبقه‌بندی دقیق برای تعیین تعرفه، محدودیت‌ها و الزامات حیاتی است. از ابزار طبقه‌بندی CBSA استفاده کنید یا با یک کارگزار گمرکی مشورت نمایید.";
        suggestions = [
          "ابزار طبقه‌بندی HS",
          "منابع CBSA",
          "مشاوره با کارگزار گمرک",
        ];
        relatedTopics = ["کدهای HS", "طبقه‌بندی کالا", "گمرک"];
      } else {
        response =
          "HS codes are 6-10 digit codes that classify products for customs purposes. Accurate classification is crucial for determining tariffs, restrictions, and requirements. Use the CBSA classification tool or consult with a customs broker.";
        suggestions = [
      ...
      messageLower.includes("restricted") ||
      messageLower.includes("prohibited")
    ) {
      if (isPersian) {
        response =
          "بسیاری از کالاها دارای محدودیت واردات یا صادرات هستند یا نیاز به مجوز ویژه دارند. فهرست کالاهای ممنوع و محدود آژانس خدمات مرزی کانادا (CBSA) را بررسی کنید و برای الزامات خاص با سازمان‌های مربوطه مانند Health Canada یا CFIA هماهنگ شوید.";
        suggestions = [
          "فهرست کالاهای ممنوع",
          "راهنمای کالاهای محدود",
          "الزامات اخذ مجوز",
        ];
        relatedTopics = ["محدودیت‌ها", "مجوزها", "مقررات"];
      } else {
        response =
          "Many products have import/export restrictions or require special permits. Check the CBSA prohibited and restricted goods list, and consult with relevant regulatory agencies (Health Canada, CFIA, etc.) for specific requirements.";
        suggestions = [
      ...
      if (isPersian) {
        response =
          "می‌توانم در زمینه‌های مختلف مربوط به حقوق تجارت بین‌الملل و انطباق گمرکی به شما کمک کنم. لطفاً درباره موضوعات مشخصی مانند رویه‌های صادرات یا واردات، الزامات مدارک، تعرفه‌ها، کدهای HS یا الزامات مقرراتی سؤال بپرسید. برای پرونده‌های پیچیده حقوقی پیشنهاد می‌کنم با یک وکیل متخصص در تجارت بین‌الملل مشورت کنید.";
        suggestions = ["رویه‌های صادرات", "الزامات واردات", "مشاوره حقوقی"];
        relatedTopics = ["حقوق تجارت بین‌الملل", "انطباق", "مشاوره حقوقی"];
      } else {
        response =
          "I can help you with various aspects of international trade law and customs compliance. Please ask about specific topics like export/import procedures, documentation requirements, tariffs, HS codes, or regulatory compliance. For complex legal matters, I recommend consulting with a qualified trade lawyer.";
        suggestions = [
          "Export Procedures",
          "Import Requirements",
          "Legal Consultation",
  ...
  static async saveChatContext(
    ...
    try {
      const { error } = await supabase.from("legal_chat_context").upsert({
      ...
      console.error("Error saving chat context:", error);
      return {
        error: `Failed to save chat context: ${
          error instanceof Error ? error.message : "Unknown error"
        }`,
      };
    }
  ...
  static async sendMessageWithMemory(
    ...
    try {
      ...
      if (configError || !aiConfig) {
        ...
        // Use direct API call without admin configuration
        const aiResponse = await this.getDirectAILegalResponse(
          message,
          contextInfo
        );
      ...
      const aiResponse = await this.getAILegalResponseWithContext(
        message,
        aiConfig,
        contextInfo
      );
   ...
   */
  private static async getDirectAILegalResponse(
    message: string,
    contextInfo: string
  ): Promise<LegalAIResponse> {
    try {
...
${this.buildLanguageDirective(message)}`,
   ...
   */
  private static async getAILegalResponseWithContext(
    message: string,
    config: any,
    contextInfo: string
  ): Promise<LegalAIResponse> {
...
${this.buildLanguageDirective(message)}`,
   ...
   */
  private static extractLegalTopics(
    userMessage: string,
    aiResponse: string
  ): string[] {
    const topics: string[] = [];
